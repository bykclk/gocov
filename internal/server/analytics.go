...
// persistence (no cookie, no localStorage), autocapture and session
// recording off, Do Not Track honoured. Signed-in users are identified by
// their gocov user id, never by email, so what PostHog holds is a numeric
// pseudonym plus page paths.
type PostHog struct {
